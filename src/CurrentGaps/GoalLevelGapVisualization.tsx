...
export const GoalLevelGapVisualization = (props: Props) => {
  const {
    targetStatuses,
    setRef,
    width,
  ...
  const statusBySDG = SDGList.map((p, i) => {
    const onTrack = targetStatuses.filter((d) => d.status === 'On Track' && p === d.goal).length;
    const identifiedGap = targetStatuses.filter((d) => d.status === 'Identified Gap' && p === d.goal).length;
    const forReview = targetStatuses.filter((d) => d.status === 'For Review' && p === d.goal).length;
    return ({
      goal: p,
      onTrack,
      identifiedGap,
      ...
      gapsNA: TargetIndicatorCount[i].noOfTargets - (onTrack + identifiedGap + forReview),
    });
  });
  useEffect(() => {
      ...
      >
        <div ref={trendRef}>
          <div className='flex-div flex-wrap'>
            {
              statusBySDG.map((d, i) => (
                <svg width='calc(16.66% - 0.8333rem)' key={i} viewBox='0 0 320 320'>
                  <g transform='translate(160, 160)'>
                    ...
                    <g transform='translate(-64,-64)'>
                      {getSDGIconSVG(`SDG ${d.goal}`, 128, false, (UNDPColorModule.sdgColors as any)[`sdg${d.goal}`])}
                    </g>
                    <circle
                      r={120}
                      cx={0}
                      cy={0}
                      style={{ fillOpacity: 0, stroke: 'var(--gray-400)', strokeWidth: 45 }}
                    />
                    <path
                      d={describeArc(0, 0, 120, 0, (d.onTrack * 360) / d.totalNoOfTargets)}
                      style={{ fillOpacity: 0, stroke: 'var(--dark-green)', strokeWidth: 45 }}
                      onMouseEnter={(event) => {
                        setHoveredSDG({
                          title: 'On Track',
                          value: d.onTrack,
                    ...
                    />
                    <path
                      d={describeArc(0, 0, 120, (d.onTrack * 360) / d.totalNoOfTargets, ((d.onTrack + d.forReview) * 360) / d.totalNoOfTargets)}
                      style={{ fillOpacity: 0, stroke: 'var(--dark-yellow)', strokeWidth: 45 }}
                      onMouseEnter={(event) => {
                        setHoveredSDG({
                          title: 'For Review',
                          value: d.forReview,
                    ...
                    />
                    <path
                      d={describeArc(0, 0, 120, ((d.onTrack + d.forReview) * 360) / d.totalNoOfTargets, ((d.onTrack + d.forReview + d.identifiedGap) * 360) / d.totalNoOfTargets)}
                      style={{ fillOpacity: 0, stroke: 'var(--dark-red)', strokeWidth: 45 }}
                      onMouseEnter={(event) => {
                        setHoveredSDG({
                          title: 'Off Track',
                    ...
                    />
                    <path
                      d={describeArc(0, 0, 120, ((d.onTrack + d.forReview + d.identifiedGap) * 360) / d.totalNoOfTargets, 360)}
                      style={{ fillOpacity: 0, stroke: 'var(--gray-400)', strokeWidth: 45 }}
                      onMouseEnter={(event) => {
                        setHoveredSDG({
                          title: 'Gaps NA',
