...
export const SDGGapsData = (props: Props) => {
  ...
  const AllSeriesForIndicator = selectedIndicator ? AllSeries.filter((d: any) => d.indicator.indexOf(`${selectedIndicator.Indicator}`.split(' ')[1]) !== -1 && selectedIndicatorTS.findIndex((el: any) => el.series === d.series) === -1).map((d) => ({
    ...d,
    values: [],
    statusCode: 6,
  })) : [];
            ...
            <SideBarButtonEl
              ...
              onClick={() => {
                ...
                if (d.Indicators.length > 0) {
                  ...
                  setSelectedIndicatorTS((countryData.filter((el) => el.indicator === d.Indicators[0].Indicator.split(' ')[1])));
                } else {
              ...
              : statusData.targetStatus[statusData.targetStatus.findIndex((el) => `Target ${el.target}` === selectedTarget.Target)].status === 'On Track'
                ...
                : statusData.targetStatus[statusData.targetStatus.findIndex((el) => `Target ${el.target}` === selectedTarget.Target)].status === 'For Review'
                  ...
                  : statusData.targetStatus[statusData.targetStatus.findIndex((el) => `Target ${el.target}` === selectedTarget.Target)].status === 'Identified Gap'
                    ...
                    : 'undp-chip-gray'}`}
          >
            {statusData.targetStatus.findIndex((el) => `Target ${el.target}` === selectedTarget.Target) === -1 ? 'Trend NA' : statusData.targetStatus[statusData.targetStatus.findIndex((el) => `Target ${el.target}` === selectedTarget.Target)].status === 'Identified Gap' ? 'Off Track' : statusData.targetStatus[statusData.targetStatus.findIndex((el) => `Target ${el.target}` === selectedTarget.Target)].status || 'Trend NA'}
          </div>
        </div>
                      ...
                      : statusData.indicatorStatus[statusData.indicatorStatus.findIndex((el) => `Indicator ${el.indicator}` === selectedIndicator.Indicator)].status === 'On Track'
                        ...
                        : statusData.indicatorStatus[statusData.indicatorStatus.findIndex((el) => `Indicator ${el.indicator}` === selectedIndicator.Indicator)].status === 'For Review'
                          ...
                          : statusData.indicatorStatus[statusData.indicatorStatus.findIndex((el) => `Indicator ${el.indicator}` === selectedIndicator.Indicator)].status === 'Identified Gap'
                            ...
                            : 'undp-chip-gray'}`}
                  >
                    {statusData.indicatorStatus.findIndex((el) => `Indicator ${el.indicator}` === selectedIndicator.Indicator) === -1 ? 'Trend NA' : statusData.indicatorStatus[statusData.indicatorStatus.findIndex((el) => `Indicator ${el.indicator}` === selectedIndicator.Indicator)].status === 'Identified Gap' ? 'Off Track' : statusData.indicatorStatus[statusData.indicatorStatus.findIndex((el) => `Indicator ${el.indicator}` === selectedIndicator.Indicator)].status || 'Trend NA'}
                  </div>
                </div>
